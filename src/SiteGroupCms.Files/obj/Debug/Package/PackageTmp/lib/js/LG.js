(function ($)
{
    ...
    {
           var p = options || {};
        var ashxUrl = options.ashxUrl || "../../ajaxhandler/ajax.aspx?";
    ...
    {
       var title=$("#searchtitle").attr("value");//文章表用
       var catalogid=$("#searchcatalogid").attr("value");//文章表用
       var state=$("#searchstate").attr("value");//文章表用
       var deptid=$("#searchdeptid").attr("value");//用户表用
       var truename=$("#searchtruename").attr("value");//真实名字用
       
       var where="1=1"
       if(title!=""&&title!=undefined)
       where+=" and title like '%"+title+"%'";
       if(catalogid!=0&&catalogid!=undefined)
       where+=" and catalogid="+catalogid;
    ...
    LG.appendSearchButtons = function (form, grid)
    {
        ...
        var container = $("<ul><li></li></ul><div class='l-clear'></div>").appendTo(searchbutton);
        LG.addSearchButtons(form, grid, container.find("li"));
        ...
        if (btn1Container)
        {
            LG.createButton({
                ...
                click: function ()
                {
                   ...
                   // if (rule.length)
                    //{
                        grid.set('parms',{ where:rule });// { where: JSON2.stringify(rule) });
                   // } else
                    //{
                    //    grid.set('parms', {});
                   // }
    ...
    LG.closeAndReloadParent = function (tabid, parentMenuNo)
    {
      ...
      //  var menuitem = $("#mainmenu ul.menulist li[menuno=" + parentMenuNo + "]");
        var menuitem = $("#jsddm li ul li[menuno=" + parentMenuNo + "]");
        //var parentTabid = menuitem.attr("tabid");
      //  var parentTabid = menuitem.attr("menuno");
